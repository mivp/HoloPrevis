    ...
    public class Node : IEnumerable<Node> {
        ...
        public void CreateGameObjects(MeshConfiguration configuration) {
            ...
            if (verticesToStore.Length <= max) {
                GameObject obj = configuration.CreateGameObject(metaData.cloudName + "/" + "r" + name + " (" + verticesToStore.Length + ")", verticesToStore, colorsToStore, boundingBox);
                if (name == "")
                {
                    GameObject gO = GameObject.FindGameObjectWithTag("PrevisModelHolder");
                    if(gO) obj.transform.parent = gO.transform;
                }
                gameObjects.Add(obj);
                ...
                while (amount > 0) {
                    ...
                    GameObject obj = configuration.CreateGameObject(metaData.cloudName + "/" + "r" + name + "_" + index + " (" + vertices.Length + ")", vertices, colors, boundingBox);
                    if (name == "")
                    {
                        GameObject gO = GameObject.FindGameObjectWithTag("PrevisModelHolder");
                        if (gO) obj.transform.parent = gO.transform;
